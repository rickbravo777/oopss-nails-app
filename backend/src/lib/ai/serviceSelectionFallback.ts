...
// than trust prompt-following alone, this is a deterministic backend safety net with five
...
// the prompt explicitly calls out as wrong. Trigger (5) is special: since the client already
// confirmed a specific service in this case, it tries to identify WHICH one by matching a
// **bolded** name in the reply against the real, already-validated services from this
// conversation (`confirmedServices`, from confirmedServices.ts) — if found, the fallback jumps
// straight to that service's especialista step, same as a correct tool call would have; if no
// match is found, it falls back to the generic full picker rather than nothing. All five
// triggers are purely additive — never remove a real tool call, never touch what was sent to
...
export function withServiceSelectionFallback(
  ...
  options: { force?: boolean; confirmedServices?: ConfirmedService[] } = {},
): ToolCallLogEntry[] {
